...
* 使用BASS内核时实现更改播放速度不变调，播放设置中增加了变调的功能
* 查找对话框中增加查找列表功能
* “播放设置”中增加“记住上次播放位置”的选项，允许在切换列表时从头播放 #834
* 新增使用QQ音乐作为下载服务的功能，常规设置中增加对应的选项
* 专辑封面下载对话框中增加右键菜单
* 专辑封面下载/在线获取标签信息对话框搜索列表增加时长显示
* 新增繁体中文语言
* 崩溃消息对话框中添加调用堆栈信息
* 选项设置中歌词文件夹、专辑封面文件和默认背景图片编辑框允许手动输入路径
* 选项设置-媒体库中新增将正在播放列表中同一首歌曲的不同版本合并的选项
* 选项设置-外观设置中新增“移除标题栏顶部边框”选项，以解决窗口标题栏顶部白色边框的问题
* 使用GDI+绘制专辑封面时，如果启用了圆角风格，将专辑封面也剪裁为圆角
* 状态栏中新增播放速度、播放变调显示
* UI中的播放列表中正在播放曲目前面绘制迷你频谱
* 调整了深色模式下，未勾选“启用背景”时的背景颜色
* 重命名和转换格式时检查目标文件路径的长度，如果路径过长则对文件名进行截断；重命名艺术家字段改回使用原始名称，不再使用第一个艺术家 #792
* 界面刷新时间间隔的最小值调整为2毫秒
* 播放设置中增加“命令行打开单个音频文件的行为”选项
* 将歌词下载到歌词文件夹时使用“艺术家-标题”的格式格式命名
* 选项设置-播放设置中增加了全屏播放时禁用屏幕休眠的选项 #868
* 关闭主窗口时新增询问是否最小化到通知区的对话框

**用户自定义界面**

* 新增文件夹浏览元素medialibFolderExplore
* 新增搜索框元素searchBox
* 界面1、界面2、界面6中，当勾选了“使用系统标准标题栏”时，右上角按钮的样式改为之前的效果，不再占用一整行
* 优化了界面2的布局
* UI中播放信息左侧的cue、播放速度标签样式改成填充的矩形背景，不再使用矩形边框
* 当界面高度小于一定值时，在不显示播放列表的情况下也显示为的small布局
* 新增界面切换器元素elementSwitcher，用于切换stackElement
* element元素增加id属性
* 导航栏元素navigationBar增加stack_element_id属性
* 导航栏元素navigationBar增加通过子元素navigationItem设置导航栏按钮的功能
* 导航栏元素navigationBar增加鼠标按下效果
* 界面“Groove音乐风格”增加点击专辑进入播放页的功能
* 解决了同一个界面中无法添加两个相同按钮元素的问题
* 新增显示面板功能，新增panel元素，按钮元素Button中的key属性增加可选值“showPanel”和“closePanel”，用于显示和关闭面板
* 按钮元素Button中的key属性增加可选值“showPlayQueue”，用于显示播放队列面板
* 按钮元素Button中的key属性增加可选值“showHideElement”，用于显示/隐藏任意一个元素
* 按钮元素Button增加text属性，支持自定义显示的文本
* 按钮元素Button增加icon属性，支持自定义显示的图标
* 按钮元素Button增加“panel_file_name”和“panel_id”属性，用于点击时显示对应的面板
* 按钮元素Button增加“related_element_id”属性，用于点击时显示/隐藏一个元素
* 堆叠元素stackElement增加“sweep_to_switch”属性，允许通过鼠标左右拖动切换界面
* 堆叠元素stackElement增加“size_change_to_switch”属性，允许设置当尺寸变化时切换显示
* 堆叠元素stackElement增加related_stack_elements属性，用于设置stackElement切换的联动
* 删除工具条toolbar元素，界面1和界面2中的工具条改为在xml中创建
* 文本元素Text增加color_style属性，允许设置文本的颜色样式
* 界面12更新，在文件夹、播放列表、我喜欢的音乐标签下增加了按钮
* 新增图标icon类型，显示为静态图标
* 界面4更新，增加了左侧导航栏
...
* 修正迷你模式播放列表无法双击播放的问题
* 修正“常规设置”中“更新源”选项没有保存到配置文件的问题 #836
* 修正歌词编辑对话框中，如果当前行没有时间标签，执行“替换时间标签”或“删除时间标签”会删除整行的问题 #835
* 修正歌词编辑对话框中，在没有翻译的情况下，执行“交换歌词和翻译”后会多出" / "的问题
* 修正媒体库对话框-文件夹标签中选中文件夹后“播放选中”按钮是灰色禁用状态的问题
* 修正wine环境下媒体库对话框文件夹浏览中树控件背景颜色不正常的问题
* 修正选项设置中专辑封面文件和默认背景图片使用相对路径时可能会无法识别的问题 #861
* 修正移除窗口顶部边框后激活窗口时标题栏区域出现闪烁的问题 #876
* 修正因网易云音乐API变动导致歌词搜索失败的问题 #887
* 修正使用FFMPEG内核时，部分音频文件在播放结束后卡住的问题 #820 #822
* 修正转换格式对话框中，因为错误导致转换未开始时，转换进度条一直显示0%的问题
* 修正选项设置-外观设置中，直接编辑“外部图片文件名”文本框，点击确认后无法保存的问题
* 修正向系统媒体传输控件传递信息时误将专辑视为艺术家传递的问题 #896
* 修正当主窗口在副显示器最大化后，最小化再点击任务栏按钮，窗口会显示到主显示器的问题 #859
* 修正了一些崩溃的问题
