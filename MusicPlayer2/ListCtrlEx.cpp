...
bool CListCtrlEx::SetRowHeight(int height, int left_space)
{
    if (height > 0 && height <= 512)
    {
        CImageList imgList;		//为ClistCtrl设置一个图像列表，以设置行高
        int width = left_space;
        if (width < 1)
            width = 1;
        BOOL rtn = imgList.Create(width, height, ILC_COLOR, 1, 1);
        if (rtn != FALSE)
        {
            m_row_height = height;
    ...
    *pResult = CDRF_DODEFAULT;
    ...
    case CDDS_ITEMPOSTPAINT:
        ...
        if (!icon_rect.IsRectEmpty())
        {
            ...
            {
                icon_rect.left += 5;
                const int icon_size{ theApp.DPI(16) };
                icon_rect.right = icon_rect.left + icon_size;
                icon_rect.top = icon_rect.top + (icon_rect.Height() - icon_size) / 2;
                icon_rect.bottom = icon_rect.top + icon_size;
                CDC* pDC = CDC::FromHandle(nmcd.hdc);
                CDrawCommon drawer;
                drawer.Create(pDC);
                drawer.DrawIcon(iter->second, icon_rect);
            }
        }
