...
namespace UiElement
{
    ...
    public:
        struct Value        //一个布局的数值
        {
            ...
            int value{ -1 };                //如果is_percentate为true，则值为百分比，否则为实际值
        ...
        Element* pParent{};     //父元素
        Element* pPreviousElement{};    //前一个元素
        std::vector<std::shared_ptr<Element>> childLst; //子元素列表

        virtual void Draw(CPlayerUIBase* ui) const;   //绘制此元素
        CRect GetRect(CPlayerUIBase* ui) const;      //计算此元素在界面中的矩形区域
    ...
    {
    public:
        virtual void Draw(CPlayerUIBase* ui) const override;
    };
    ...
    public:
        ...
        virtual void Draw(CPlayerUIBase* ui) const override;
        void FromString(const std::wstring& key_type);
    };
...
{
public:
    std::shared_ptr<UiElement::Element> CreateElement(const std::wstring& name);
};
