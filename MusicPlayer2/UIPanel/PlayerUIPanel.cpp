    ...
    if (m_root_element != nullptr)
    {
        ...
        if (!IsFullFill())
        {
            ...
            if (theApp.m_app_setting_data.button_round_corners && draw_round_background)
                m_ui->GetDrawer().DrawRoundRect(m_panel_rect, m_ui->GetUIColors().color_panel_back, m_ui->CalculateRoundRectRadius(m_panel_rect), alpha);
            else
                m_ui->GetDrawer().FillAlphaRect(m_panel_rect, m_ui->GetUIColors().color_panel_back, alpha, true);
        }
